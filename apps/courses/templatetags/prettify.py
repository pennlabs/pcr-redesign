...
# @register.filter(name='alias_sort')
# def alias_sort(reviews):
#     return dictsort:"section.course.coursehistory.alias"
...
def average(courses, attribute):
    average = 0.0
    valid = 0
    f = ReviewBit.objects.filter(field=attribute).filter(review__section__course__in=courses)
    for reviewbit in f:
        try:
            average += reviewbit.score
            valid += 1
        except KeyError:
            continue
    try:
